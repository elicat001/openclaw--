...
const COOKIE_JAR = "/tmp/amazon-br-cookies.txt";
...
export const amazonBr: CrawlerModule = {
  name: "amazon-br",
  crawl: async (opts: CrawlOptions): Promise<RawProduct[]> => {
    ...
    console.log("[amazon-br] Warming up...");
    await curlFetch("https://www.amazon.com.br/").catch(() => {});
    await sleep(1500);

    for (let page = 1; page <= 20; page++) {
      const url = `https://www.amazon.com.br/s?k=${encodeURIComponent(opts.keyword)}&page=${page}`;
      console.log(`[amazon-br] Page ${page}...`);
      try {
    ...
    for (let i = 0; i < asins.length; i += BATCH_SIZE) {
      ...
      console.log(
        `[amazon-br] Batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(asins.length / BATCH_SIZE)} (${batch.length} ASINs)...`,
      );
    ...
    console.log(
      `\n[amazon-br] Done: ${results.length} products | avg ${avgImages.toFixed(1)} images/product | ${withSkus} with SKUs`,
    );
